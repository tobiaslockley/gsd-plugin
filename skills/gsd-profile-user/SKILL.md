...
- @${CLAUDE_PLUGIN_ROOT}/agents/gsd-user-profiler.md (profiler agent definition)
...
```bash
PROFILE_PATH="${CLAUDE_PLUGIN_ROOT}/USER-PROFILE.md"
...
```bash
cp "${CLAUDE_PLUGIN_ROOT}/USER-PROFILE.md" "${CLAUDE_PLUGIN_ROOT}/USER-PROFILE.backup.md"
```
...
✓ Stores profile at ${CLAUDE_PLUGIN_ROOT}/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node ${CLAUDE_PLUGIN_ROOT}/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node ${CLAUDE_PLUGIN_ROOT}/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `${CLAUDE_PLUGIN_ROOT}/references/user-profiling.md`
...
**Get questions:**
```bash
QUESTIONS=$(node ${CLAUDE_PLUGIN_ROOT}/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node ${CLAUDE_PLUGIN_ROOT}/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to ${CLAUDE_PLUGIN_ROOT}/USER-PROFILE.md"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at ${CLAUDE_PLUGIN_ROOT}/USER-PROFILE.md" and jump to step 10.
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```
...
```bash
BACKUP_PATH="${CLAUDE_PLUGIN_ROOT}/USER-PROFILE.backup.md"
```
...
Your profile:    ${CLAUDE_PLUGIN_ROOT}/USER-PROFILE.md
```
