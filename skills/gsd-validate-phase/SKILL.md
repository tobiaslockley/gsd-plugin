...
<!-- Workflow content (was: validate-phase.md) -->
<purpose>
...
AGENT_SKILLS_AUDITOR=$(node "$GSD_TOOLS" agent-skills gsd-nyquist-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(node "$GSD_TOOLS" resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node "$GSD_TOOLS" config-get workflow.nyquist_validation --raw)
```
...
**State B (create):**
1. Read template from `${CLAUDE_PLUGIN_ROOT}/templates/VALIDATION.md`
...
node "$GSD_TOOLS" commit "docs(phase-${PHASE}): add/update validation strategy"
```
