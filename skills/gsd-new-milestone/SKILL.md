...
<!-- Workflow content (was: new-milestone.md) -->
<purpose>
...
```bash
node "$GSD_TOOLS" phases clear
```

```bash
node "$GSD_TOOLS" commit "docs: start milestone v[X.Y] [Name]" --files .planning/PROJECT.md .planning/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(node "$GSD_TOOLS" init new-milestone)
...
**Commit requirements:**
```bash
node "$GSD_TOOLS" commit "docs: define milestone v[X.Y] requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
node "$GSD_TOOLS" commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
...
<process>
Execute the new-milestone workflow steps embedded above end-to-end.
