...
<!-- Workflow content (was: execute-phase.md) -->
<purpose>
...
```bash
INIT=$(node "$GSD_TOOLS" init execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "$GSD_TOOLS" agent-skills gsd-executor 2>/dev/null)
```
...
```bash
USE_WORKTREES=$(node "$GSD_TOOLS" config-get workflow.use_worktrees 2>/dev/null || echo "true")
```
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  node "$GSD_TOOLS" config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Follow the plan execution workflow **inline**
...
```bash
node "$GSD_TOOLS" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "$GSD_TOOLS" phase-plan-index "${PHASE_NUMBER}")
```
...
5.6. **Post-wave shared artifact update (worktree mode only):**
   ...
   for PLAN_ID in ${WAVE_PLAN_IDS}; do
     node "$GSD_TOOLS" roadmap update-plan-progress "${PHASE_NUMBER}" "${PLAN_ID}" completed
   done
...
7b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "$GSD_TOOLS" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
AUTO_CFG=$(node "$GSD_TOOLS" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
```bash
POST_PLAN_INDEX=$(node "$GSD_TOOLS" phase-plan-index "${PHASE_NUMBER}")
```
...
```bash
PARENT_INFO=$(node "$GSD_TOOLS" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node "$GSD_TOOLS" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
SCHEMA_DRIFT=$(node "$GSD_TOOLS" verify schema-drift "${PHASE_NUMBER}" 2>/dev/null)
```
...
```bash
VERIFIER_SKILLS=$(node "$GSD_TOOLS" agent-skills gsd-verifier 2>/dev/null)
```
...
Commit the file:
```bash
node "$GSD_TOOLS" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
```bash
COMPLETION=$(node "$GSD_TOOLS" phase complete "${PHASE_NUMBER}")
```
...
```bash
node "$GSD_TOOLS" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
```bash
node "$GSD_TOOLS" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
<process>
Execute the execute-phase workflow steps embedded above end-to-end.
