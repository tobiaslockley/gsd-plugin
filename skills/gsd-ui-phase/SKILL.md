...
<!-- Workflow content (was: ui-phase.md) -->
<purpose>
...
AGENT_SKILLS_UI=$(node "$GSD_TOOLS" agent-skills gsd-ui-researcher 2>/dev/null)
AGENT_SKILLS_UI_CHECKER=$(node "$GSD_TOOLS" agent-skills gsd-ui-checker 2>/dev/null)
```
...
```bash
UI_RESEARCHER_MODEL=$(node "$GSD_TOOLS" resolve-model gsd-ui-researcher --raw)
UI_CHECKER_MODEL=$(node "$GSD_TOOLS" resolve-model gsd-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(node "$GSD_TOOLS" config-get workflow.ui_phase 2>/dev/null || echo "true")
```
...
```bash
node "$GSD_TOOLS" commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
