...
<!-- Workflow content (was: docs-update.md) -->
<purpose>
...
```bash
INIT=$(node "$GSD_TOOLS" docs-init)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "$GSD_TOOLS" agent-skills gsd-doc-writer 2>/dev/null)
```
...
```bash
node "$GSD_TOOLS" commit "docs: generate project documentation" \
...
<process>
Execute the docs-update workflow steps embedded above end-to-end.
