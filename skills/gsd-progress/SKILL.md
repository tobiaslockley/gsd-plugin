...
<!-- Workflow content (was: progress.md) -->
<purpose>
...
```bash
INIT=$(node "$GSD_TOOLS" init progress)
...
- `STATE=$(node "$GSD_TOOLS" state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node "$GSD_TOOLS" summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$(node "$GSD_TOOLS" progress bar --raw)
```
...
```bash
DEBT=$(node "$GSD_TOOLS" audit-uat --raw 2>/dev/null)
```
...
```bash
PHASE_SECTION=$(node "$GSD_TOOLS" roadmap get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
```bash
NEXT_PHASE_SECTION=$(node "$GSD_TOOLS" roadmap get-phase "$((Z+1))" 2>/dev/null)
...
<process>
Execute the progress workflow steps embedded above end-to-end.
