...
<!-- Workflow content (was: map-codebase.md) -->
<purpose>
...
```bash
INIT=$(node "$GSD_TOOLS" init map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(node "$GSD_TOOLS" agent-skills gsd-codebase-mapper 2>/dev/null)
```
...
```bash
node "$GSD_TOOLS" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
