...
<!-- Workflow content (was: autonomous.md) -->
<purpose>
...
```bash
INIT=$(node "$GSD_TOOLS" init milestone-op)
...
```bash
ROADMAP=$(node "$GSD_TOOLS" roadmap analyze)
```
...
```bash
DETAIL=$(node "$GSD_TOOLS" roadmap get-phase ${PHASE_NUM})
```
...
```bash
PHASE_STATE=$(node "$GSD_TOOLS" init phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(node "$GSD_TOOLS" config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
node "$GSD_TOOLS" commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_SECTION=$(node "$GSD_TOOLS" roadmap get-phase ${PHASE_NUM} 2>/dev/null)
...
```bash
UI_REVIEW_CFG=$(node "$GSD_TOOLS" config-get workflow.ui_review 2>/dev/null || echo "true")
```
...
```bash
node "$GSD_TOOLS" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
<process>
Execute the autonomous workflow steps embedded above end-to-end.
