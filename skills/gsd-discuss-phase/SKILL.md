...
<!-- Workflow content (was: discuss-phase.md) -->
<purpose>
...
AGENT_SKILLS_ADVISOR=$(node "$GSD_TOOLS" agent-skills gsd-advisor 2>/dev/null)
```
...
```bash
TODO_MATCHES=$(node "$GSD_TOOLS" todo match-phase "${PHASE_NUMBER}")
```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "$GSD_TOOLS" resolve-model gsd-advisor-researcher --raw)
   ```
...
```bash
MAX_PASSES=$(node "$GSD_TOOLS" config-get workflow.max_discuss_passes 2>/dev/null || echo "3")
```
...
```bash
node "$GSD_TOOLS" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "$GSD_TOOLS" state record-session \
...
```bash
node "$GSD_TOOLS" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
If `DISCUSS_MODE` is `"assumptions"`: Follow the assumptions discussion workflow embedded above.

If `DISCUSS_MODE` is `"discuss"` (or unset, or any other value): Follow the standard discussion workflow embedded above.

**MANDATORY:** The workflow content embedded above IS the complete instruction set. Follow the step-by-step process with all required behaviors, config checks, and interaction patterns. Do not improvise from the summary sections alone.
</process>
