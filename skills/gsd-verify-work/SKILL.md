...
<!-- Workflow content (was: verify-work.md) -->
<purpose>
...
```bash
INIT=$(node "$GSD_TOOLS" init verify-work "${PHASE_ARG}")
...
```
UI_PHASE_FLAG=$(node "$GSD_TOOLS" config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```bash
CHECKPOINT=$(node "$GSD_TOOLS" uat render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
node "$GSD_TOOLS" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
<process>
Execute the verify-work workflow steps embedded above end-to-end.
