...
<!-- Workflow content (was: plan-milestone-gaps.md) -->
<purpose>
...
HIGHEST=$(node "$GSD_TOOLS" phases list --pick directories[-1])
```
...
```bash
node "$GSD_TOOLS" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
<process>
Execute the plan-milestone-gaps workflow steps embedded above end-to-end.
